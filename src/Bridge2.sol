...
contract Bridge2 is Pausable, ReentrancyGuard {
  ...
  function requestWithdrawal(
    ...
    console.log("hotValidatorSet.validators.length:", hotValidatorSet.validators.length);
    ...
    if (!isValidWithdrawal(message)) {
      console.log("ERROR: Invalid withdrawal (invalidated)");
    ...
    if (requestedWithdrawals[message].requestedTime != 0) {
      console.log("ERROR: Withdrawal already requested at timestamp:", requestedWithdrawals[message].requestedTime);
      emit FailedWithdrawal(message, 0);
      return;
    }
    console.log("Calling checkValidatorSignatures...");
    checkValidatorSignatures(message, hotValidatorSet, signatures, hotValidatorSetHash);
    console.log("checkValidatorSignatures passed!");
  ...
  function checkValidatorSignatures(
    ...
    console.log("=== checkValidatorSignatures START ===");
    console.logBytes32(message);
    console.log("activeValidatorSet.validators.length:", activeValidatorSet.validators.length);
    console.log("signatures.length:", signatures.length);
    console.logBytes32(validatorSetHash);
    console.log("totalValidatorPower:", totalValidatorPower);
    
    bytes32 computedHash = makeValidatorSetHash(activeValidatorSet);
    console.logBytes32(computedHash);
    require(
      computedHash == validatorSetHash,
      "Supplied active validators and powers do not match the active checkpoint"
    );
    console.log("Validator set hash matches!");
    ...
    console.log("Starting signature verification loop...");
    for (uint64 activeValidatorSetIdx; activeValidatorSetIdx < end; activeValidatorSetIdx++) {
      address expectedValidator = activeValidatorSet.validators[activeValidatorSetIdx];
      uint64 validatorPower = activeValidatorSet.powers[activeValidatorSetIdx];
      console.log("Checking validator index:", activeValidatorSetIdx);
      console.logAddress(expectedValidator);
      console.log("power:", validatorPower);
      console.log("Using signature index:", signatureIdx);
      ...
      if (signer == expectedValidator) {
        console.log("MATCH! Adding power:", validatorPower);
        cumulativePower += validatorPower;
        console.log("Cumulative power:", cumulativePower);
        uint64 requiredPower = (2 * totalValidatorPower) / 3;
        console.log("Required power (2/3):", requiredPower);

        if (3 * cumulativePower > 2 * totalValidatorPower) {
          console.log("Quorum met! Breaking loop.");
          break;
        }

        signatureIdx += 1;
        if (signatureIdx >= nSignatures) {
          console.log("All signatures processed, breaking loop.");
          break;
        }
      } else {
        console.log("NO MATCH - continuing to next validator without advancing signatureIdx");
      }
    }

    console.log("Final cumulativePower:", cumulativePower);
    uint64 requiredPowerFinal = (2 * totalValidatorPower) / 3;
    console.log("Required power (2/3):", requiredPowerFinal);
    console.log("Check: 3 * cumulativePower > 2 * totalValidatorPower");
    uint64 checkValueFinal = 3 * cumulativePower;
    uint64 checkRequiredFinal = 2 * totalValidatorPower;
    console.log("Check value:", checkValueFinal);
    console.log("Check required:", checkRequiredFinal);
    require(
      3 * cumulativePower > 2 * totalValidatorPower,
      "Submitted validator set signatures do not have enough power"
    );
    console.log("=== checkValidatorSignatures SUCCESS ===");
  }
  ...
  function depositWithPermit(
    ...
    console.log("=== depositWithPermit START ===");
    console.logAddress(user);
    console.log("usd:", usd);
    ...
    try usdcToken.transferFrom(user, spender, usd) returns (bool success) {
      ...
      if (!success) {
        console.log("transferFrom returned false");
        emit FailedPermitDeposit(user, usd, 1);
        return;
      }
      // Emit Deposit event on successful transfer
      console.log("Deposit successful! Emitting Deposit event");
      emit Deposit(user, usd);
    } catch {
      console.log("transferFrom failed!");
      emit FailedPermitDeposit(user, usd, 1);
    }
    console.log("=== depositWithPermit END ===");
  }
